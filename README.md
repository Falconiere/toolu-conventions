...
| 2. Agent hooks | `.claude/settings.json` | On every file an agent writes (`PostToolUse`) and again before it finishes a turn (`Stop`) |
...
└── __tests__/            # fixtures + plugin + latency suites, on real trees
```
...
- cross-stack template references **resolve** (`marketing` and `expo` deliberately copy
  from `console` — one source, on purpose);
...
| [`guardrails/`](./guardrails/) | `agent-guardrails` — the shared structural gate, copied into every project |
| [`lint/`](./lint/) | Shared oxlint/oxfmt core, copied byte-identically into every TS stack's `templates/`. Not distributed on its own |
| [`stacks/`](./stacks/) | The five stack kits |
