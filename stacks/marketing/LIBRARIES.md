...
| Lint / format | `oxlint` + `oxfmt` (+ `oxlint-tsgolint` for type-aware) | TypeScript files only — `.astro` is `astro check`'s job. |
...
| Styling (opt-in) | `tailwindcss` + `@tailwindcss/vite` | Only if the design context asks for it. The theme tokens stay the source of truth — use `globals.tailwind.css` from the console kit. |
