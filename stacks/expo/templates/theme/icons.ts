// Icon set — CodaSignal "Signal" language. 60 marks, no icon library. Import
// directly: import { icons } from '@/ui/theme/icons';
//
// Glyphs annotate; icons afford. Where a control needs a target rather than a
// note, use this set. Every mark is drawn on a 24 box with a 20 live area, 1.25pt
...
// DESIGN.md. Identical to the web kit's file, so a mark cannot drift per platform.
//
// Values are the inner markup of a `<svg viewBox="0 0 24 24">`. Render through
// react-native-svg's `SvgXml` with one wrapper that owns the stroke, the width
// and the caps/joins — so a mark can never drift from the construction rules:
//
//   <SvgXml
//     width={size} height={size}
//     xml={`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
//            stroke="${color}" stroke-width="${iconStroke[size]}"
//            stroke-linecap="square" stroke-linejoin="miter">${icons[name]}</svg>`}
//   />
