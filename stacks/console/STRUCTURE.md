...
rules below — it never relaxes a CORE rule. Designed for **clean domain
...
│   │   ├── index.tsx         # `/` — points at a domain screen
...
│   │   └── queries/          # hooks shared by several domains (optional)
...
> Folder vocabulary (`ui`, `utilities`, `api`, `domains`, `providers`) is
...
both the build and the tests. Always import via the `@/` alias or same-directory `./` — never parent-relative
paths like `../x` or `../../ui`.
...
| `@/domains/*` | `src/domains/*` | `import { HomeScreen } from '@/domains/home/screens/home-screen'` |
...
the conventional shape, not separate config entries. Parent-relative imports
(`../…` and deeper) are a lint error (`no-restricted-imports`); use `@/` or `./`.
...
1. **`src/app/` is routes only, and thin.** A file under `src/app/` declares a
   `Route` that maps a URL to a domain screen and configures the route (loader,
   ...
   `@/domains/*`, so domain logic can't leak into other layers) + review.
...
5. **Two call paths, no third.** Our own API goes through `orpc`
   ...
   no bare `fetch` scattered through domains — auth headers, base URL, timeouts
...
6. **Zod at every boundary, types inferred from it.** Env, any response `http`
   ...
   review.
7. **Nothing unused, nothing duplicated.** TypeScript and oxlint reject unused
   locals and parameters, including names prefixed with `_`; `knip` rejects an
   unused file, export, or dependency; `jscpd` rejects copy-paste. Delete or wire
   dead code instead of suppressing the checks.
   _Enforced by:_ `noUnusedLocals`, `noUnusedParameters`,
   `eslint/no-unused-vars`, `knip.json`, and `.jscpd.json` (`threshold: 0` is
   what fails the duplication gate; `exitCode: 1` covers a later threshold).
...
Domain hooks like the two above live in `src/domains/<name>/hooks/` when
one domain owns them, or `src/api/queries/` when several do.
