/** Color tokens — CodaSignal "Signal" language. Plain TS, no styling library. Import: import { colors } from '@/ui/theme/colors'. */

// Two bands + one signal. The rationale lives in the kit's DESIGN.md; this file
// is the concrete source of truth.
//
// A page is a stack of full-bleed BANDS that alternate dark and light. Each band
// owns its own token set and no component straddles the seam, so every primitive
// must read correctly in both. `palette` is the raw ramp — components never read
// it. They read the semantic maps: `colors` (dark band, the default) or
// `colorsLight` (light band).
//
...
export const palette = {
  ...
  inkMutedDark: '#8a8d86',
  inkDark: '#f2f2ef',
  inkHoverDark: '#d0d1ce', // ink @ 86% over the dark band — the primary hover fill
  ...
  inkLight: '#16170f',
  inkHoverLight: '#2c2e24', // the primary hover fill on a light band
...
export interface SignalTheme {
  signal: string;
  /** One step up — hover, and the signal ink on a light band's lift state. */
  lift: string;
...
/**
 * The shipped default, and the value every TS consumer of `colors` reads. The
 * CSS seam themes via `<html data-signal="…">`, which does NOT touch this file —
 * set both, or `colors.accent` in TS silently stays Jade while the page renders
 * another temperature.
 */
...
/** Semantic token contract. Both bands implement it, so primitives swap band by map. */
export interface SemanticColors {
  /** The band itself — full-bleed, owns the section. */
  background: string;
  /** Floating panel on the band: card, product shot, form group. */
  surface: string;
  /** Recessed inside a panel: field fill, code block, terminal. */
  surfaceInset: string;
  /** Row hover. The only background change any list row makes. */
  surfaceHover: string;

  text: string;
  textMuted: string;
  /** Sub-copy under a row title, nav links, glyph arrows. */
  textSoft: string;
  /**
  ...
  success: string;
  /** Slow, retrying, over budget. Dot, border or 10px mono — never a fill. */
  warning: string;
  ...
  borderInner: string;
  /** Secondary-button border, unchecked box, key cap. */
  borderStrong: string;
...
export const colors: SemanticColors = {
  ...
  primaryHover: palette.inkHoverDark,
...
export const colorsLight: SemanticColors = {
  ...
  primaryHover: palette.inkHoverLight,
...
export const shadow = {
  /** Cards, form groups. */
  card: '0 18px 44px rgba(20, 22, 15, 0.07)',
  /** The standard product-shot panel. */
  panel: '0 24px 60px rgba(20, 22, 15, 0.09)',
  /** The hero panel, once per page. */
  panelLg: '0 28px 70px rgba(20, 22, 15, 0.1)',
} as const;
