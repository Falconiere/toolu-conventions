...
| Lint | Cargo `[lints]` + `clippy` + guardrails | `dead_code = "deny"` rejects unused items; `lint-suppressions` rejects `allow(dead_code)` attributes; Clippy owns the remaining house set and CI runs `-D warnings`. |
